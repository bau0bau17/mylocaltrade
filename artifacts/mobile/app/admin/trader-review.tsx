...
export default function AdminTraderReviewScreen() {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const { token, isAdmin } = useAuth();
  const params = useLocalSearchParams<{ status?: string }>();
  const initialStatus =
    typeof params.status === 'string' && STATUS_FILTERS.some((f) => f.key === params.status)
      ? params.status
      : 'UNDER_REVIEW';
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.light.background }}>
      <Stack.Screen options={{ title: 'Trader Review Queue' }} />

      <View style={[styles.headerRow, { paddingTop: Math.max(insets.top, 50) + 12 }]}>
        <Pressable onPress={() => router.back()} style={styles.backBtn}>
          <Feather name="arrow-left" size={20} color={Colors.light.text} />
        </Pressable>
        <Text style={styles.headerTitle}>Trader Review Queue</Text>
        <View style={{ width: 36 }} />
      </View>
