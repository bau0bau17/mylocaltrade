...
// ---------------------------------------------------------------------------
// Dark mode variant of the refined Admin dashboard — same layout, nav
// structure, orange accent and typography, adapted to a dark surface
// palette. This is a visual mockup only; there is no live API wired in, so
// the numbers below are representative sample data.
// ---------------------------------------------------------------------------
...
const STATUS_STYLES: Record<string, string> = {
  UNDER_REVIEW: "bg-[hsl(var(--add-warning-tint))] text-[hsl(var(--add-warning))]",
  NEEDS_MORE_INFO: "bg-[hsl(var(--add-warning-tint))] text-[hsl(var(--add-warning))]",
  VERIFIED: "bg-[hsl(var(--add-success-tint))] text-[hsl(var(--add-success))]",
  REJECTED: "bg-[hsl(var(--add-danger-tint))] text-[hsl(var(--add-danger))]",
};
...
const toneText: Record<string, string> = {
  muted: "text-[hsl(var(--add-muted))]",
  info: "text-[hsl(var(--add-info))]",
  warning: "text-[hsl(var(--add-warning))]",
  success: "text-[hsl(var(--add-success))]",
  danger: "text-[hsl(var(--add-danger))]",
};

const toneBg: Record<string, string> = {
  muted: "bg-[hsl(var(--add-muted-bg))]",
  info: "bg-[hsl(var(--add-info))]",
  warning: "bg-[hsl(var(--add-warning))]",
  success: "bg-[hsl(var(--add-success))]",
  danger: "bg-[hsl(var(--add-danger))]",
};

const activityToneBg: Record<string, string> = {
  success: "bg-[hsl(var(--add-success-tint))] text-[hsl(var(--add-success))]",
  danger: "bg-[hsl(var(--add-danger-tint))] text-[hsl(var(--add-danger))]",
  info: "bg-[hsl(var(--add-info-tint))] text-[hsl(var(--add-info))]",
  muted: "bg-[hsl(var(--add-muted-bg))] text-[hsl(var(--add-muted))]",
};
...
function StatCard({
  ...
  return (
    <div
      className={`add-card add-card-hover add-animate rounded-xl p-4 cursor-pointer ${
        highlight ? "ring-1 ring-[hsl(var(--add-primary)/0.35)]" : ""
      }`}
      ...
      <div className="flex items-start justify-between">
        <div
          className={`w-9 h-9 rounded-lg flex items-center justify-center ${
            highlight
              ? "bg-[hsl(var(--add-primary-tint))] text-[hsl(var(--add-primary))]"
              : "bg-[hsl(var(--add-muted-bg))] text-[hsl(var(--add-muted))]"
          }`}
        >
          <Icon className="w-4 h-4" />
        </div>
        {delta && (
          <span className="flex items-center gap-0.5 text-[11px] font-semibold text-[hsl(var(--add-success))] add-mono">
            <ArrowUpRight className="w-3 h-3" />
            {delta}
          </span>
        )}
      </div>
      <div className="mt-3 text-[28px] font-extrabold leading-none add-mono tracking-tight">{value}</div>
      <div className="mt-1.5 text-[13px] font-semibold text-[hsl(var(--add-foreground))]">{label}</div>
      <div className="mt-0.5 text-xs text-[hsl(var(--add-muted))]">{hint}</div>
    </div>
  );
}
    ...
    <div className={`add-root add-theme-transition min-h-[100dvh] w-full flex relative ${isLight ? "light" : ""}`}>
      ...
      <aside className="add-sidebar hidden md:flex md:flex-col w-64 shrink-0 text-[hsl(var(--add-sidebar-fg))] relative z-10">
        <div className="px-5 py-5 flex items-center gap-2.5 border-b border-[hsl(var(--add-sidebar-border))]">
          <div className="bg-[hsl(var(--add-primary))] text-[hsl(var(--add-primary-foreground))] rounded-lg w-8 h-8 flex items-center justify-center shrink-0">
        ...
        <nav className="add-scrollbar flex-1 px-3 py-4 space-y-5 overflow-y-auto">
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`add-nav-item w-full flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] font-medium ${
                          active ? "active" : ""
                        }`}
                      >
                        ...
                        {!!item.count && (
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[hsl(var(--add-primary))] text-[hsl(var(--add-primary-foreground))] add-mono">
                            {item.count}
                          </span>
                        )}
                      </button>
        ...
        <div className="p-3 border-t border-[hsl(var(--add-sidebar-border))]">
      ...
      <div className="flex-1 min-w-0 flex flex-col relative z-10">
        {/* Mobile header */}
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-[hsl(var(--add-card))] border-b border-[hsl(var(--add-card-border))]">
          <div className="flex items-center gap-2 font-bold text-sm">
            <Shield className="w-4 h-4 text-[hsl(var(--add-primary))]" /> MyLocalTrade Admin
          </div>
          <button type="button" className="w-8 h-8 rounded-md flex items-center justify-center text-[hsl(var(--add-muted))]">
        ...
        <main className="flex-1 overflow-y-auto p-5 md:p-8 space-y-6">
          {/* Page header */}
          <div className="flex items-start justify-between flex-wrap gap-3 add-animate">
            <div>
              <h1 className="text-[26px] font-extrabold tracking-tight">Dashboard</h1>
              <p className="text-sm text-[hsl(var(--add-muted))] mt-0.5">
                Operational overview at a glance.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <div className="hidden sm:flex items-center gap-1.5 text-xs text-[hsl(var(--add-muted))] bg-[hsl(var(--add-card))] border border-[hsl(var(--add-card-border))] rounded-full px-3 py-1.5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="add-card add-animate rounded-xl lg:col-span-1" style={{ animationDelay: "200ms" }}>
              <div className="px-4 pt-4 pb-2 flex items-center justify-between">
                <h2 className="text-[15px] font-bold">Trader status</h2>
                <span className="text-xs text-[hsl(var(--add-muted))] add-mono">{totalTraders} total</span>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[13px] font-medium group-hover:text-[hsl(var(--add-primary))] transition-colors">
                          {s.label}
                        </span>
                        <span className="text-[13px] font-semibold add-mono">{s.count}</span>
                      </div>
                      <div className="add-progress-track h-1.5 rounded-full overflow-hidden">
                        <div
            ...
            <div className="add-card add-animate rounded-xl lg:col-span-2" style={{ animationDelay: "240ms" }}>
              <div className="px-4 pt-4 pb-2 flex items-center justify-between">
                <h2 className="text-[15px] font-bold">Recent activity</h2>
                <button type="button" className="text-xs font-semibold text-[hsl(var(--add-primary))] flex items-center gap-0.5 hover:gap-1.5 transition-all">
              ...
              <div className="px-2 pb-2">
                {ACTIVITY.map((entry) => {
                  ...
                  return (
                    <div
                      key={entry.id}
                      className="flex items-center gap-3 px-2.5 py-2.5 rounded-lg hover:bg-[hsl(var(--add-muted-bg))] transition-colors"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-[hsl(var(--add-muted))] truncate">{entry.who}</div>
                      </div>
                      <div className="text-[11px] text-[hsl(var(--add-muted))] whitespace-nowrap add-mono">
                        {entry.time}
                      </div>
                    </div>
                  );
          ...
          <div className="add-card add-animate rounded-xl overflow-hidden" style={{ animationDelay: "280ms" }}>
            <div className="px-4 pt-4 pb-3 flex items-center justify-between flex-wrap gap-2">
              <div>
                ...
                <p className="text-xs text-[hsl(var(--add-muted))] mt-0.5">Newest submissions needing a decision.</p>
              </div>
              <div className="relative">
                <Search className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-[hsl(var(--add-muted))]" />
                <input
                  placeholder="Search traders…"
                  className="text-xs pl-8 pr-3 py-1.5 rounded-lg border border-[hsl(var(--add-card-border))] bg-[hsl(var(--add-muted-bg))] text-[hsl(var(--add-foreground))] placeholder:text-[hsl(var(--add-muted))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--add-primary)/0.3)] w-44"
                  readOnly
                />
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-t border-[hsl(var(--add-card-border))] text-[10px] uppercase tracking-wider text-[hsl(var(--add-muted))]">
                ...
                <tbody className="divide-y divide-[hsl(var(--add-card-border))]">
                  {TRADERS_PREVIEW.map((t) => (
                    <tr key={t.name} className="hover:bg-[hsl(var(--add-muted-bg))] transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          {t.risk && (
                            <span
                              className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                                t.risk === "high" ? "bg-[hsl(var(--add-danger))]" : "bg-[hsl(var(--add-warning))]"
                              }`}
                            />
                          )}
                          <div className="min-w-0">
                            <div className="font-semibold truncate">{t.name}</div>
                            <div className="text-xs text-[hsl(var(--add-muted))] truncate">{t.category}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-xs text-[hsl(var(--add-muted))] hidden sm:table-cell">{t.contact}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold ${STATUS_STYLES[t.status]}`}>
                          {STATUS_LABELS[t.status]}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-xs text-[hsl(var(--add-muted))] hidden md:table-cell add-mono">{t.submitted}</td>
                      <td className="px-4 py-3 text-right">
                        <button
                          type="button"
                          className="inline-flex items-center gap-1 text-xs font-semibold text-[hsl(var(--add-primary))] px-2.5 py-1.5 rounded-md hover:bg-[hsl(var(--add-primary-tint))] transition-colors"
                        >
            ...
            <div className="px-4 py-3 border-t border-[hsl(var(--add-card-border))] flex items-center justify-between">
              <p className="text-xs text-[hsl(var(--add-muted))]">Showing 4 of 23 traders</p>
              <button
                type="button"
                className="text-xs font-semibold border border-[hsl(var(--add-card-border))] rounded-md px-3 py-1.5 hover:bg-[hsl(var(--add-muted-bg))] transition-colors"
              >
                Load more
              </button>
            </div>
