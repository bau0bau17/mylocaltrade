...
## Phase C (confirmed products, 2026-08-15)
- Five CONFIRMED App Store Connect products (group "Trader Subscription"): trader.monthly/.yearly (Solo) + trader.team5/.team10/.team20 .yearly (note `.trader.` segment — the old invented ids lacked it). RevenueCat `default` offering packages: `$rc_monthly`, `$rc_annual`, `team_5_annual/team_10_annual/team_20_annual`; entitlement id is exactly `Trader Subscription` (case+space).
- Config lives in shared env (`.replit [env]`, applies dev+prod): TEAM_PRODUCT_SEAT_MAP JSON with the three team ids, EXPO_PUBLIC_REVENUECAT_ENTITLEMENT_ID="Trader Subscription" (also in all three eas.json profiles). No backend code change was needed — the Phase B resolver reads the env map.
- Mobile display tier resolution (`resolveTeamTier` in lib/revenuecat.tsx) must stay EXACT-id: offering package match first, then an exact known-id map. Architect rejected a substring heuristic — `team50`/`team15`-shaped unknown ids must fall back to the generic "Premium" label, mirroring the server's fail-closed seat resolver.
- Pricing screen with an ACTIVE sub now shows a change-plan list (all five, current marked); plan switches are Apple in-group upgrades/downgrades via normal `purchase()` — never a second subscription.
- `TEAM_BILLING_ENFORCED` remains false (Phase D = enforcement rollout, NOT started; user drives it).
