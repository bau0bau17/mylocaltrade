...
- Testing policy: run the isolated automated test suites (vitest workflows) freely, without asking. However, ALWAYS ask before any test/check that touches production or real external resources: volume/load tests, manual or smoke tests against production services, real API keys (e.g. Brevo, Twilio), the production database, or real user email addresses/phone numbers. Tests must NEVER send real emails or consume the Brevo daily quota — keep the transport-stripping in `artifacts/api-server/src/test-setup.ts` and the reserved-test-domain guard in `src/lib/email.ts` intact; the ~300 Brevo emails/day are reserved exclusively for live users.
- Build & release ownership: the user personally runs Git sync on their Mac, EAS builds, and TestFlight submissions — always manually. The agent must NEVER start an EAS build, submit to TestFlight, or toggle environment feature flags (development or production, e.g. `COMPANY_TEAMS_ENABLED`) on its own initiative. Deliver mobile changes only via commit + push to `main`; the user takes it from there.
